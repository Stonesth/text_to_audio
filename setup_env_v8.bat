...
set "INCLUDE=%MSVC_PATH%\%MSVC_VERSION%\include;%MSVC_PATH%\%MSVC_VERSION%\ATLMFC\include;%MSVC_PATH%\%MSVC_VERSION%\include\msvc;%VS_PATH%\VC\Auxiliary\VS\include;%INCLUDE%"
...
REM Vérification/Installation de Python 3.10
...
pip uninstall TTS -y
pip install TTS==0.17.6 --no-deps --only-binary :all:
if errorlevel 1 (
